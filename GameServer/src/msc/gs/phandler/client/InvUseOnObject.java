...
import msc.gs.external.ItemCookingDef;
import msc.gs.external.ItemCraftingDef;
import msc.gs.external.ItemSmeltingDef;
import msc.gs.external.ItemSmithingDef;
import msc.gs.external.ItemWieldableDef;
...
import msc.gs.plugins.quests.Dorics;
...
public class InvUseOnObject implements PacketHandler {
	...
	Instance.getDelayedEventHandler().add(new WalkToObjectEvent(player, object, false) {
		...
		owner.resetAll();
		String[] options;
		if ((object.getID() == Dorics.ANVIL1_ID && object.getX() == Dorics.ANVIL1_X && object.getY() == Dorics.ANVIL1_Y) || (object.getID() == Dorics.ANVIL2_ID && object.getX() == Dorics.ANVIL2_X && object.getY() == Dorics.ANVIL2_Y)) {
		    if (owner.getQuestStage(7) != Quest.COMPLETE) {
			Npc npc = World.getWorld().getNpc(Dorics.DORIC_ID, 323, 327, 487, 492);

			if (npc == null) {
			    player.getActionSender().sendMessage("You need to complete Doric's quest to use his anvils");
			    player.setBusy(false);
			    return;
			} else {
			    owner.informOfNpcMessage(new ChatMessage(npc, "Oi! Get off my anvil!", owner));
			    Instance.getWorld().getDelayedEventHandler().add(new MiniEvent(owner) {
				public void action() {
				    player.getActionSender().sendMessage("You need to complete Doric's quest to use his anvils");
				    player.setBusy(false);
				    return;
		...
		world.getQuestManager().handleAction(QuestAction.ITEM_USED_ON_OBJECT, new Object[] { item, object }, owner);
		...
		if (item.getID() == 132) {
			...
			Instance.getDelayedEventHandler().add(new MiniEvent(owner, 2000) {
			    public void action() {
		...
		if (object.getX() == 233 && object.getY() == 180) {
			...
			if (owner.getInventory().remove(new InvItem(414)) > -1) {
			    ...
			    Instance.getDelayedEventHandler().add(new MiniEvent(owner, 1000) {
				public void action() {
				    owner.setBusy(false);
		...
		case 302: // Sandpit
		    ...
		    owner.setBusy(true);
		    showBubble();
		    Instance.getDelayedEventHandler().add(new MiniEvent(owner) {
			public void action() {
			    if (owner.getInventory().remove(item) > -1) {
		...
		private void handleRegularSmelting() {
	    	handleRegularSmelting((int)Math.ceil(owner.getMaxStat(13) / 10));
	    	...
	    	final int tries = --passedvalue;
	    	 if (item.getID() == 622) { // Seaweed (Glass)
	 			...
	 			Instance.getDelayedEventHandler().add(new ShortEvent(owner) {
	 				...
	 				owner.setBusy(false);
	 				if(tries > 0 && owner.getInventory().contains(item)) {
	 					handleCooking(tries);
	 				}
	 			    	}
	 			});
		...
		if (owner.getCurStat(13) < def.getRequiredLevel()) {
		    owner.getActionSender().sendMessage("You need a smithing level of " + def.getRequiredLevel() + " to make this");
		    return;
		}
		...
		if (EntityHandler.getItemDef(def.getItemID()).isMembers() && !World.isMembers()) {
