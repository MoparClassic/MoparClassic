...
public class DuelHandler implements PacketHandler {
	...
	case 87: // Confirm accepted
    	...
    	world.addEntryToSnapshots(new Activity(player.getUsername(), player.getUsername() + " started duel " + affectedPlayer.getUsername() + " at: " + player.getX() + "/" + player.getY() + " | " + affectedPlayer.getX() + "/" + affectedPlayer.getY()));
		...
		WalkToMobEvent walking = new WalkToMobEvent(player, affectedPlayer, 1) {
		    public void arrived() {
			Instance.getDelayedEventHandler().add(new SingleEvent(owner, 300) {
			    public void action() {
				...
				Instance.getDelayedEventHandler().add(dueling);
			    }
			});
		    }
