...
public class PickupItem implements PacketHandler {
	...
	if(!item.getDef().canTrade()) {
		if(item.droppedby() != 0 && msc.gs.tools.DataConversions.usernameToHash(player.getUsername()) != item.droppedby()) {
	...
	Instance.getDelayedEventHandler().add(new WalkToPointEvent(player, location, 1, false) {
		...
		if (pickedUpRare) {
		    ...
		    final long playerLong = msc.gs.tools.DataConversions.usernameToHash(owner.getUsername());
		...
		if(item.getAmount() > 10000000 || Formulae.isRareItem(item.getID()))
			DBConnection.getReport().submitDupeData(owner.getUsername(),owner.getUsernameHash());
