...
import msc.gs.event.FightEvent;
import msc.gs.event.ObjectRemover;
import msc.gs.event.WalkMobToMobEvent;
import msc.gs.event.WalkToMobEvent;
import msc.gs.event.WalkToPointEvent;
import msc.gs.external.EntityHandler;
import msc.gs.external.ItemSmeltingDef;
import msc.gs.external.ReqOreDef;
...
public class SpellHandler implements PacketHandler {
	...
	case 33:
	    ...
	    Instance.getDelayedEventHandler().add(new ObjectRemover(guthix, 500));
	    break;
	case 34:
	...
	case 35:
	    ...
	    Instance.getDelayedEventHandler().add(new ObjectRemover(zammy, 500));
	    break;
	case 47:
	    GameObject charge = new GameObject(affectedMob.getLocation(), 1147, 0, 0);
	    world.registerGameObject(charge);
	    Instance.getDelayedEventHandler().add(new ObjectRemover(charge, 500));
	    break;
	}
    }
	...
	case 43: // Enchant lvl-5 dragonstone amulet
	    if(affectedItem.getID() == 610) { if(!checkAndRemoveRunes(player,
		    spell)) { return; } player.getInventory().remove(affectedItem);
		    player.getInventory().add(new InvItem(522));
		    finalizeSpell(player, spell); } else {
			player.getActionSender().sendMessage
			("This spell cannot be used on this kind of item"); } break;
	...
	Instance.getDelayedEventHandler().add(new WalkToPointEvent(player, affectedItem.getLocation(), 5, true) {
	    public void arrived() {
		owner.resetPath();
		...
		/*
		 * int itemid = affectedItem.getID(); if(itemid == 828 || itemid
		 * == 831 || itemid == 832 || itemid == 422 || itemid == 1289) {
		 * owner.getActionSender().sendMessage(
		 * "Using Telekinetic grab on rare drops isn't fun ):"); return;
		 * }
		 */
	...
	Instance.getDelayedEventHandler().add(new WalkToMobEvent(player, affectedMob, 5) {
	    public void arrived() {
		...
		case 19: // Crumble undead
			...
			if (owner instanceof Player) {
				...
				for (Damager fig : ((Npc) affectedMob).getSyndicate().getDamagers()) {
				    if (fig.isUseMagic() && !fig.isUseCombat())
					continue;
					...
					if (fig.isUseRanged()) {
					    fig.getPlayer().incExp(4, newXP * 3, true);
					...
					case 0:
					    for (int x = 0; x < 3; x++) {
						fig.getPlayer().incExp(x, newXP, true);
						fig.getPlayer().getActionSender().sendStat(x);
					    }
					    break;
					case 1:
					    fig.getPlayer().incExp(2, newXP * 3, true);
					    fig.getPlayer().getActionSender().sendStat(2);
					    break;
					case 2:
					    fig.getPlayer().incExp(0, newXP * 3, true);
					    fig.getPlayer().getActionSender().sendStat(0);
					    break;
					case 3:
					    fig.getPlayer().incExp(1, newXP * 3, true);
					    fig.getPlayer().getActionSender().sendStat(1);
					    break;
					}
					fig.getPlayer().incExp(3, newXP, true);
		...
		case 33:
		    ...
		    else{owner.getActionSender().sendMessage("You need to be wearing the Staff of Guthix to cast this spell!");return;}
		case 34:
		    if(affectedMob instanceof Npc) {
			if(((Npc)affectedMob).getID() == 477) {
			    player.getActionSender().sendMessage("The dragon seems immune to this spell");
			...
			if(cape.getID() == 1218 && cape.isWielded()) {bool=bool||true;}
			//else {flag = false;}
		    }
		    if(bool){
			if(!owner.isCharged()){owner.getActionSender().sendMessage("@red@You are not charged!");}
		    ...
		    else{owner.getActionSender().sendMessage("You need to be wearing the Staff of Saradomin to cast this spell!");return;}
		case 35:
		    if(affectedMob instanceof Npc) {
			if(((Npc)affectedMob).getID() == 477) {
			    player.getActionSender().sendMessage("The dragon seems immune to this spell");
			...
			if(cape.getID() == 1216 && cape.isWielded()) {flag2=flag2||true;}
			//else {flag = false;}
		    }
		    if(flag2){
			if(!owner.isCharged()){owner.getActionSender().sendMessage("@red@You are not charged!");}
		    ...
		    else{owner.getActionSender().sendMessage("You need to be wearing the Staff of Zaramorak to cast this spell");return;}

		default:
		    if(spell.getReqLevel() == 62 || spell.getReqLevel() == 65 || spell.getReqLevel() == 70 || spell.getReqLevel() == 75) {
			if(!Constants.GameServer.MEMBER_WORLD) {
			    player.getActionSender().sendMessage("Must be on a members server to use this");
			    return;
			}
			...
			npcc.getSyndicate().addDamage(owner, damage, true, false, false);
		    }
			...
			if (np.isScripted())
			    Instance.getPluginHandler().getNpcAIHandler(np.getID()).onHealthPercentage(np, percent);
		    }
			...
			if (owner instanceof Player) {
			  Player toLoot = owner;
				...
				for (Damager fig : ((Npc) affectedMob).getSyndicate().getDamagers()) {
				    ...
				    if (fig.isUseMagic() && !fig.isUseCombat())
					continue;
				    // if(fig.player != attackerPlayer)
				    /*
				     * for(Player p : attackerPlayer.getViewArea
				     * ().getPlayersInView()) if(p ==
				     * fig.player) found = true; if(!found) //
				     * skip a person who is not in the area.
				     * continue;
				     */
	...
	case 71: // Cast on npc
		...
		if (affectedNpc.inCombat() && World.getQuestManager().isNpcAssociated(affectedNpc, player)) {
		    if (player.getOpponent() != null && player.getOpponent().equals(affectedNpc)) {
		    } else {
			player.getActionSender().sendMessage("You can't cast spells on the " + affectedNpc.getDef().getName() + " while it's in combat!");
			player.resetPath();
			return;
		    }
		}
