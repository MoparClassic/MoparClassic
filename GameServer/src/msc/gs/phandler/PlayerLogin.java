...
import msc.gs.phandler.client.WieldHandler;
...
public class PlayerLogin implements PacketHandler {
    ...
    public void handlePacket(msc.gs.connection.Packet p, IoSession session) throws Exception {
	byte loginCode = p.readByte();
	if(world.getPlayer(player.getUsernameHash()) != null) {
		Exception e = new Exception("Double log from " + player.getUsername() + " | " + player.getUsernameHash()); 
		e.printStackTrace();
		loginCode = 5;
		player.destroy(true);
	...
	if (loginCode == 0 || loginCode == 1 || loginCode == 99) {
	    ...
	    msc.gs.builders.MiscPacketBuilder sender = player.getActionSender();
		...
		if (i.isWielded() && (i.getID() == 407 || i.getID() == 401)) {
		    ...
		    if (player.getCurStat(6) < 31 || count <= World.getQuestManager().getQuests().size() - 1) {
			player.getActionSender().sendMessage("You must have at least 31 magic & completed " + (World.getQuestManager().getQuests().size() - 1) + " quests");
	    ...
	    if (player.clientWarn()) {
			...
			player.getActionSender().sendMessage("@red@Alert! @whi@You are using an old client, please download the new client from our website. This client WILL stop working on @red@soon.");
	    }
