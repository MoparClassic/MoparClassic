...
public class MiscPacketBuilder {
    ...
    this.sendMessage("@yel@Players Online: @whi@" + (int)(Instance.getWorld().getPlayers().size() * 1.1) + " @yel@Accepted Connections: @whi@" + Constants.GameServer.ACCEPTED_CONNECTIONS);
    }
	...
	for (int i = 0; i < size; i++) {
	    ...
	    Integer objectInteger = player.getQuestStages().get(quest.getUniqueID());
   ...
   /* public void sendWakeUp1() {
	RSCPacketBuilder s = new RSCPacketBuilder();
	s.setID(224);
	packets.add(s.toPacket());
    }

    public void sendWakeUp2() {
	player.isSleeping = false;
	RSCPacketBuilder s = new RSCPacketBuilder();
	s.setID(224);
	packets.add(s.toPacket());
	...
	for (InvItem i : shop.getItems()) {
	    ...
	    s.addInt(Formulae.getPrice(shop.getItems().get(Formulae.getItemPos(shop, i.getID())), shop, true));
	    s.addInt(Formulae.getPrice(shop.getItems().get(Formulae.getItemPos(shop, i.getID())), shop, false));
	}
	packets.add(s.toPacket());
    }
    /**
