...
public abstract class Quest {
     ...
     * Used for triggers
     */
    public abstract void handleAction(QuestAction action, Object[] args, Player player);
    ...
    public void associateObject(int id, int x, int y) {
    	associatedObjects.put(new java.awt.Point(id, associatedObjects.size()), Point.location(x, y));
    }
    ...
    public void associateObject(int id) {
    	associatedObjects.put(new java.awt.Point(id, associatedObjects.size()), Point.location(0, 0));
    }
    ...
    public boolean isObjectAssociated(GameObject obj, Player player) {
		...
		for (java.awt.Point p : set) {
		    int i = (int) p.getX();
		    if (i == obj.getID())
			if (Point.location(obj.getX(), obj.getY()).equals(associatedObjects.get(p)) || (associatedObjects.get(p).getX() == 0 && associatedObjects.get(p).getY() == 0)) {
				Logger.println("obj id " + i + " is associated with quest " + getName());
			    return true;
			} else
				Logger.println("obj id " + i + " is NOT associated with quest " + getName());
		}
		return false;
    }
