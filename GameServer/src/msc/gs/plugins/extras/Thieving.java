...
public class Thieving {
    ...
    public static String[] StealChats = { "You think i'm going to buy my own items off you? Get out of here.", "You've just stolen from me, I'm not selling you anything.", "How dare you try and buy from me after you have stolen.", "Thief, go away now before i call security", "You have guts, trying to sell my own goods back to me", "Real thiefs don't sell back to their victims", "i know better than to buy my own items from thiefs" };
    ...
    private String[] caughtChats = { "Guards! Guards! Im being Robbed!", "Help Guards i am being Robbed please help!", "Someone help! My items are getting stolen!", "You'll wish you never did that, Thief!", "You are going to pay for that", "-name- how could you steal from me? Guards!", "-name- get your hands out of my stall!", "Hey -name- thats not yours!", "Dont steal from me -name- Im going to go tell a mod", "Oi! -name- you deserve a spanking!" };
    private String[] Chats = { "Oi! Get your hands out of there -name-!", "Hey thief! get here!", "Trying to steal from me hmm?", "No one steals from me!", "Take those hands off me Thief", "Are you trying to steal from me -name-?", "Dont you dare touch me", "Thief get back here now!", "Stealing won't get you anywhere", "Bad person! you shall die", "Die evil thief!", "You are going to pay for that", "Ill make you wish you were never born", "-name- i am going to hurt you", "-name- dont steal from me again", "Remove your filthy hands off me", "A real man doesn't need to steal" };
	...
	try {
	    ...
	    Logger.println("Player " + player.getUsername() + " found a NPC (pickpocket) not added, ID: " + npcID);
		ret = true;
	    }
	    ...
	    Instance.getDelayedEventHandler().add(new WalkToMobEvent(player, affectedMob, 1) {
		public void arrived() {
		    if (owner.getSpam()) {
			return;
		    } else {
			...
			if (owner.getCurStat(17) < lvl) {
			    owner.getActionSender().sendMessage("You must be at least " + lvl + " thieving to pick the " + affectedNpc.getDef().name + "'s pocket.");
			    owner.setBusy(false);
			    // affectedNpc.unblock();
			    owner.setBusy(false);
			    owner.setSpam(false);
			...
			Instance.getDelayedEventHandler().add(new ShortEvent(owner) {
				...
				if (chanceFormulae(lvl)) {
				    ...
				    owner.incExp(17, exp * ExpMultiplier, true);
					...
					public void action() {
						...
						owner.setBusy(false);
						owner.resetPath();
						affectedNpc.setBusy(false); // untick
						// this
						return;
					    }
	...
	try {
		...
		Logger.println("Player " + player.getUsername() + " found a door(lockpick) not added, ID: " + object.getID() + ", Coords: " + object.getLocation());
		player.setSpam(false);
		player.setBusy(false);
		return;
	    }
		...
		public void action() {
			...
			doDoor();
			owner.incExp(17, Doors[curDoor][5] * ExpMultiplier, true);
			owner.getActionSender().sendStat(17);
			owner.setSpam(false);
			owner.setBusy(false);
	...
	if (ourChest == -1) {
	    ...
	    Logger.println("Player " + player.getUsername() + " found a chest not added, ID: " + object.getID() + ", Coords: " + object.getLocation());
	    return;
	}
	...
	Instance.getDelayedEventHandler().add(new MiniEvent(player, 300) {
		...
		Instance.getDelayedEventHandler().add(new MiniEvent(player, 1000) {
		    public void action() {
			...
			Instance.getDelayedEventHandler().add(new MiniEvent(player, 1000) {
				...
				Instance.getDelayedEventHandler().add(new MiniEvent(player, 1200) {
					...
					if (Chests[ourChest][0] == 340) {
					    owner.getActionSender().sendMessage("You triggered the trap!");
					    owner.teleport(612, 568, true);
					}
					owner.getActionSender().sendInventory();
	...
	try {
		...
		Logger.println("Player " + player.getUsername() + " found a stall not added, ID: " + object.getID() + ", Coords: " + object.getLocation());
		player.setSpam(false);
		return;
	    }
		...
		public void action() {
			...
			owner.getActionSender().sendInventory();
			owner.incExp(17, Stalls[curStall][2] * ExpMultiplier, true);
			owner.getActionSender().sendStat(17);
		    }
		}
	    });
