...
import msc.gs.model.Script;
import msc.gs.model.World;
import msc.gs.quest.Quest;
import msc.gs.quest.QuestAction;
...
public class KnightsSword extends Quest {
	...
	public void handleAction(QuestAction action, Object[] args, final Player player) {
		...
		if (action == action.TALKED_NPC) {
			...
			if (npc.getID() != NPC_SQUIRE && npc.getID() != NPC_RELDO && npc.getID() != NPC_DWARF && npc.getID() != NPC_VYVIN) {
				return;
			}
		...
		} else if (action == action.USED_OBJECT) {
			if (!(args[0] instanceof GameObject)) {
				return;
			}

			final GameObject obj = (GameObject) args[0];
			final boolean click = (Boolean) args[1];
			if (obj.getID() != CUPBOARD_ID && obj.getID() != BLUERITE_ID) {
				return;
			}

			if (obj.getID() == CUPBOARD_ID && obj.getX() == CUPBOARD_X && obj.getY() == CUPBOARD_Y) {
				player.setBusy(true);

				final Npc npc = World.getWorld().getNpc(138, 316, 320, 2454, 2459);
				...
				if (npc != null) {
					...
					if (hasItem(player, PICTURE_ID) || player.getQuestStage(this) < 4) {
						player.getActionSender().sendMessage("The cupboard is just full of junk");
						sleep(1200);
						player.setBusy(false);
						return;
					}
	 ...
	 * msc.gs.external.EntityHandler.getObjectMiningDef(102);
	 *
	 ...
	 *
	 * int axeId = -1; for(int id : msc.config.Formulae.miningAxeIDs) {
	 * if(countItem(player, id) > 0) { axeId = id; break; } }
	 *
	 * if(axeId < 0) {
	 ...
	 *
	 * if(msc.config.Formulae.getOre(def, player.getCurStat(14), axeID)) {
	 * giveItem(player, ore);
	...
	private void handleVyvinChat(final Npc npc, final Player player) {
		...
		player.setBusy(false);
		int option = getMenuOption(player, "Do you have anything to trade?", "Why are there so many knights in this city?");
		if (option == -1) {
			return;
		}
		player.setBusy(true);
		...
		if (option == 0) {
			...
			queueChat(npc, player, "We are the white knights of falador", "We are the most powerfull order of knights in the land", "We are helping the king vallance rule the kingdom", "As he is getting old and tired");
			player.setBusy(false);
			npc.unblock();
			return;
		}
	...
	private void handleDwarfChat(final Npc npc, final Player player) {
		...
		if (stage == 2) {
			player.setBusy(false);
			int option = getMenuOption(player, "Hello are you an incando Dwarf?", "Would you like some redberry pie?");
			if (option == -1) {
				return;
			}
			player.setBusy(true);
			...
			if (option == 0) {
				...
				player.setBusy(false);
				option = getMenuOption(player, "Would you like some redberry pie?", "Can you make me a special sword?");
				if (option == -1) {
					return;
				}
				player.setBusy(true);
			...
			sendChat("I need you to make a sword for one of falador's knights", player, npc);
			sendChat("He had one which was passed down through five generations", player, npc);
			sendChat("But his squire has lost it", player, npc);
			queueChat(npc, player, "A knight's sword eh?", "Well i'd need to know exactly how it looked", "Before i could make a new one", "All the faladian knights used to have swords with different designs", "Could you bring me a picture or something?");
			...
			if (hasItem(player, PICTURE_ID)) {
				sendChat("I have found a picture of the sword i would like you to make", player, npc);
				player.getActionSender().sendMessage("You hand Thurgo the portrait");
				takeItem(player, PICTURE_ID);
				sleep(2000);
				player.getActionSender().sendMessage("Thurgo examines the picture for a moment");
				sleep();
				...
				sendChat("Have you got a picture of the sword for me yet?", npc, player);
			...
			if (hasItem(player, 265)) {
				sendChat("Thanks for your help getting the sword for me", player, npc);
			...
			if (countItem(player, 170) >= 2 && hasItem(player, BLUERITE_ORE_ID)) {
				...
				player.getActionSender().sendMessage("You hand Thurgo the items");
				takeItem(player, new InvItem(170), new InvItem(BLUERITE_ORE_ID), new InvItem(170));
				player.getActionSender().sendMessage("Thurgo hammers the materials into a metal sword");
				sleep();
				giveItem(player, SWORD_ID);
				sendChat("Here you go", npc, player);
				sendChat("Thank you very much", player, npc);
				sendChat("Just remember to call in with more pie some time", npc, player, 1200);
				player.setBusy(false);
				npc.unblock();
				return;
			} else {
				...
				sendChat("Remember i need blurite ore and two iron bars", npc, player, 1200);
				player.setBusy(false);
				npc.unblock();
				return;
			}
	...
	private void givePie(final Npc npc, final Player player) {
		...
		if (!hasItem(player, 258)) {
			sendChat("Well that's too bad, because I don't have any", player, npc);
			player.getActionSender().sendMessage("Thurgo does not look impressed");
			sleep(1200);
			player.setBusy(false);
			npc.unblock();
			return;
			...
			sendChat("Anyone who makes pie like that has gotta be alright", npc, player, 1200);
			player.setBusy(false);
			npc.unblock();
			return;
		}
	...
	private void handleReldoChat(final Npc npc, final Player player) {
		...
		if (stage == 1) {
			...
			player.setBusy(false);
			int option = getMenuOption(player, "I'm in search of a quest", "Do you have anything to trade?", "What do you do?", "What do you know about the incando dwarves?");
			if (option == -1) {
				return;
			}
			player.setBusy(true);
	...
	private void handleSquireChat(final Npc npc, final Player player) {
		...
		if (stage == -1) {
			...
			player.setBusy(false);
			int option = getMenuOption(player, "And how is life as a squire?", "Wouldn't you prefer to be a squire for me?");
			if (option == -1) {
				return;
			}
			player.setBusy(true);
			...
			if (option == 0) {
				...
				player.setBusy(false);
				option = getMenuOption(player, "Do you know where you lost it?", "I can make a new sword if you like", "Is he angry?");
				if (option == -1) {
					return;
				}
				player.setBusy(true);
				...
				if (option == 0) {
					...
					queueChat(npc, player, "He doesn't know yet", "I was hoping i could think of something to do", "Before he does find out", "But i find myself at a loss");
			...
			sendChat("But he needs a picture of the sword before he can make it", player, npc);
			...
			sendChat("Sir vyvin keeps it in a cupboard in his room i think", npc, player, 1200);
			...
			if (hasItem(player, SWORD_ID)) {
				...
				player.getActionSender().sendMessage("You give the sword to the squire");
			...
			queueChat(npc, player, "Hello friend", "Thanks for your help before", "Vyvin never even realised it was a different sword");
			player.setBusy(false);
			npc.unblock();
			return;
		}
	...
	private void mainOptionsChat(final Npc npc, final Player player) {
		player.setBusy(false);
		int option = getMenuOption(player, "Well do you know the vague area you lost it?", "I can make a new sword if you like", "Well the kingdom is fairly abundant with swords", "Well I hope you find it soon");
		if (option == -1) {
			return;
		}
		player.setBusy(true);
	...
	private void abundantSwordChat(final Npc npc, final Player player) {
		queueChat(npc, player, "Yes you can get bronze swords anywhere", "But this isn't any old sword");

		dwarfChat(npc, player);
	}

	private void dwarfChat(final Npc npc, final Player player) {
		queueChat(npc, player, "The thing is, this sword is a family heirloom", "It has been passed down through vyvin's family for five generations", "It was originally made by the incando dwarves", "Who were a particularly skilled tribe of dwarven smiths", "I doubt anyone could make it in the style they do");

		player.setBusy(false);
		int option = getMenuOption(player, "So would these dwarves make another one?", "Well I hope you find it soon");
		if (option == -1) {
			return;
		}
		player.setBusy(true);
	...
	private void lostItChat(final Npc npc, final Player player) {
		queueChat(npc, player, "No i was carrying it for him all the way from where he had it stored in Varrock", "It must have slipped from my pack during the trip", "And you know what people are like these days", "Someone will have just picked it up and kept it for themselves");

		player.setBusy(false);
		int option = getMenuOption(player, "I can make a new sword if you like", "Well the kingdom is fairly abundant with swords", "Well I hope you find it soon");
		if (option == -1) {
			return;
		}
		player.setBusy(true);
	...
	private void findItChat(final Npc npc, final Player player) {
		...
		sendChat("I'm not looking forward to telling vyvin i've lost it", npc, player);
		sendChat("He's going to want it for the parade next week as well", npc, player, 1200);
		player.setBusy(false);
		npc.unblock();
		return;
	}
	...
	private void startQuest(final Npc npc, final Player player) {
		...
		if (option == 0) {
			...
			sendChat("As i say the best place to start should be with reldo", npc, player, 1200);
