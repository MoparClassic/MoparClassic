...
public class WitchsPotion extends Quest {
	...
	if (action == action.TALKED_NPC) {
	    ...
	    if (npc.getID() == RAT_ID) {
		if (stage == 1)
		    world.registerItem(new Item(TAIL_ID, npc.getX(), npc.getY(), 1, player));
	    }
	    ...
	    if (obj.getID() == CAULDRON_ID && obj.getX() == CAULDRON_X && obj.getY() == CAULDRON_Y) {
		player.setBusy(true);
	...
	if (stage == -1) {
	    ...
	    sendChat("What could you want with an old woman like me?", npc, player);

	    player.setBusy(false);
	    int option = getMenuOption(player, "I am in search of a quest", "I've heard that you are a witch");
	    if (option == -1)
		return;
	    player.setBusy(true);
	    sleep();
	    ...
	    if (option == 0) {
		sendChat("Hmm maybe i can think of something for you", npc, player);

		sendChat("Would you like to become more proficient in the dark arts?", npc, player);

		player.setBusy(false);
		option = getMenuOption(player, "Yes help me become one with my darker side", "No I have my principles and honour", "What, you mean improve my magic?");
		if (option == -1)
		    return;
		player.setBusy(true);
		sleep();
		...
		if (option == 0) {
		    ...
		    sendChat("Suit yourself, but you're missing out", npc, player);
		    sleep(1200);
		    player.setBusy(false);
		    npc.unblock();
		    return;
		    ...
		    player.setBusy(false);
		    option = getMenuOption(player, "Yes I'd like to improve my magic", "No I'm not interested", "Show me the mysteries of the dark arts");
		    if (option == -1)
			return;
		    player.setBusy(true);
		    sleep();
			...
			sendChat("But i think you'll be drawn back to this place", npc, player, 1200);
			player.setBusy(false);
			npc.unblock();
			return;
		    } else {
		...
		sendChat("Yes it does seem to be getting fairly common knowledge", npc, player);
		sendChat("I fear i may get a visit from the witch hunters of falador before long", npc, player, 1200);
		player.setBusy(false);
		npc.unblock();
		return;
	    }
	} else if (stage == 1) {
	    sendChat("So have you found the things for the potion?", npc, player);
		...
		sleep();
		sendMessage(player, "Hetty puts all the ingredients in her cauldron");
		sleep();
		...
		sendChat("An eye of newt, a rat's tail, some burnt meat and an onion", npc, player, 1200);
		player.setBusy(false);
		npc.unblock();
		return;
	    }
	} else if (stage == 2) {
	    sendChat("Well are you going to drink the potion or not?", npc, player, 1200);
	    player.setBusy(false);
	    npc.unblock();
	    return;
	...
	sendChat("Ok, I'm going to make a potion to help bring out your darker self", npc, player);
	sendChat("So that you can perform acts of dark magic with greater ease", npc, player);
	...
	if (!dark) {
	    ...
	    player.setBusy(false);
	    int option = getMenuOption(player, "No, I don't like the sound of it", "Well, alright...");
	    if (option == -1)
		return;
	    player.setBusy(true);
	    sleep();
	...
	sendChat("You need an eye of newt, a rat's tail, an onion and a piece of burnt meat", npc, player);
