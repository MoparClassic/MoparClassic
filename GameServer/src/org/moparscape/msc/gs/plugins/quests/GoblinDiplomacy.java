...
import org.moparscape.msc.gs.event.*;
import org.moparscape.msc.gs.model.*;
import org.moparscape.msc.gs.quest.*;
...
public class GoblinDiplomacy extends Quest {
	...
	if (action == QuestAction.TALKED_NPC) {
	    ...
	    if ((item1.getID() == ORANGE_DYE_ID && item2.getID() == GOBLIN_ARMOUR_ID) || (item1.getID() == GOBLIN_ARMOUR_ID && item2.getID() == ORANGE_DYE_ID)) {
		player.setBusy(true);
		...
		return;
	    } else if ((item1.getID() == BLUE_DYE_ID && item2.getID() == GOBLIN_ARMOUR_ID) || (item1.getID() == GOBLIN_ARMOUR_ID && item2.getID() == BLUE_DYE_ID)) {
		player.setBusy(true);
		...
		player.setBusy(false);
		return;
	    } else if ((item1.getID() == RED_DYE_ID && item2.getID() == YELLOW_DYE_ID) || (item1.getID() == YELLOW_DYE_ID && item2.getID() == RED_DYE_ID)) {
		player.setBusy(true);
		sendMessage(player, "You mix the two dyes and make an orange dye");
		sleep(1200);
		takeItem(player, item1, item2);
	...
	player.setBusy(false);
	int option = getMenuOption(player, "What could you make for me", "Cool, do you turn people into frogs?", "You mad old witch, you can't help me", "Can you make dyes for me please");
	if (option == -1)
	    return;
	player.setBusy(true);
	sleep();
	...
	if (option == 0) {
	    ...
	    sendChat("I sometimes make dye for the womens clothes, brighten the place up", npc, player);
	    sendChat("I can make red, yellow and blue dyes. would you like some?", npc, player);

	    player.setBusy(false);
	    option = getMenuOption(player, "What do you need to make some red dye please", "What do you need to make some yellow dye please", "What do you need to make some blue dye please", "No thanks, I am happy the colour I am");
	    if (option == -1)
		return;
	    player.setBusy(true);
	    sleep();
		...
		sendChat("You are easily pleased with yourself then", npc, player);
	    ...
	    sendChat("Oh, not for years, but if you meet a talking chicken,", npc, player);
	    sendChat("You have probably met the professor in the manor north of here", npc, player);
	    sendChat("A few years ago it was flying fish, that machine is a menace", npc, player, 1200);
	    player.setBusy(false);
	    npc.unblock();
	    return;
	} else if (option == 2) {
		...
		sendMessage(player, "Aggie waves her hands about, and you seem to be 20 coins poorer");
		takeItem(player, new InvItem(10, 20));
		sleep();
		sendChat("Thats a fine for insulting a witch, you should learn some respect", npc, player, 1200);
	    } else {
		sendChat("You should be careful about insulting a witch", npc, player);
		sendChat("You never know what shape you could wake up in", npc, player, 1200);
	    }
	    ...
	    sendChat("What sort of dye would you like? red, yellow or blue?", npc, player);

	    player.setBusy(false);
	    option = getMenuOption(player, "What do you need to make some red dye please", "What do you need to make some yellow dye please", "What do you need to make some blue dye please", "None thanks");
	    if (option == -1)
		return;
	    player.setBusy(true);
	    sleep();
	...
	sendChat("Yellow is a strange colour to get, comes from onion skins", npc, player);
    ...
    private void handleDye(String colour, int itemId, int amount, String itemName, int dyeId, final Npc npc, final Player player) {
	player.setBusy(false);
	int option = getMenuOption(player, "Okay, make me some " + colour + " dye please", "I don't think I have all the ingredients yet", "I can do without dye at that price");
	if (option == -1)
	    return;
	player.setBusy(true);
	sleep();
	...
	if (option == 0) {
		...
		sendChat("Ah wait, sorry. I don't have all the ingredients yet", player, npc);
		sendChat("You know what you need to get now, come back when you have them", npc, player);
		sendChat("Goodbye for now", npc, player, 2000);
		player.setBusy(false);
		npc.unblock();
		sendMessage(player, "You don't have enough " + itemName + " for the dye!");
		return;
	    }

	    if (countItem(player, 10) < 5) {
		sendChat("Ah wait, sorry, I don't have enough coins on me", player, npc);
		sendChat("Well come back when you do and I'll make you some dye", npc, player, 2000);
	    ...
	    sendMessage(player, "You hand the " + itemName + " and payment to Aggie");
	    player.getActionSender().sendSound("coin");
	    takeItem(player, new InvItem(10, 5), new InvItem(itemId, 1));
	    sleep();
	    sendMessage(player, "She takes a red bottle from nowhere and hands it to you");
	    ...
	    sendChat("That's your choice, but I would think you have killed for less", npc, player);
	...
	player.setBusy(false);
	int option = getMenuOption(player, "I'm looking for woad leaves", "Not right now thanks");
	if (option == -1)
	    return;
	player.setBusy(true);
	sleep();
	...
	if (option == 0) {
	    sendChat("Well luckily for you I may have some around here somewhere", npc, player);
	    sendChat("Can I buy one please?", player, npc);
	    sendChat("How much are you willing to pay?", npc, player);

	    player.setBusy(false);
	    option = getMenuOption(player, "How about 5 coins?", "How about 10 coins?", "How about 15 coins?", "How about 20 coins?");
	    if (option == -1)
		return;
	    player.setBusy(true);
	    sleep();
	    ...
	    {
		sendChat("No no thats far too little. Woad leaves are hard to get you know", npc, player);
		sendChat("I used to have plenty but someone kept stealing them off me", npc, player, 1200);
		player.setBusy(false);
		npc.unblock();
		return;
	    }
		...
		if (countItem(player, 10) < 15) {
		    sendChat("I dont have enough coins to buy the leaves. I'll come back later", player, npc, 1200);
		    player.setBusy(false);
		    npc.unblock();
		    return;
		}
		...
		sendMessage(player, "Wyson the gardener gives you some woad leaves");
		sleep(1200);
		player.setBusy(false);
		npc.unblock();
	    } else // 20
		...
		sendMessage(player, "Wyson the gardener gives you some woad leaves");
		sleep();
		sendChat("Here have some more you're a generous person", npc, player);
		takeItem(player, WOAD_LEAF_ID);
		sendMessage(player, "Wyson the gardener gives you some more leaves");
		sleep(1200);
		player.setBusy(false);
		npc.unblock();
		return;
	...
	if (stage == -1) {
	    ...
	    player.setBusy(false);
	    int option = getMenuOption(player, "Could I buy a beer please?", "Not very busy in here today is it?");
	    if (option == -1)
		return;
	    player.setBusy(true);
	    sleep();
		...
		if (countItem(player, 10) >= 2) {
		    ...
		    player.getActionSender().sendMessage("You don't have enough coins to buy the beer!");
		    sleep(1200);
		    player.setBusy(false);
		    npc.unblock();
		    return;
		...
		queueChat(npc, player, "No it was earlier", "There was a guy in here saying the goblins up by the mountain", "Are arguing again", "Of all things about the colour of their armour.", "Knowing the goblins, it could easily turn into a full blown war", "Which wouldn't be good", "Goblin wars make such a mess of the countryside");
		sendChat("Well if I have time I'll see if I can go and knock some sense into them", player, npc, 1200);
	    ...
	    player.setBusy(false);
	    int option = getMenuOption(player, "Could I buy a beer please?", stage == COMPLETE ? "I fixed the goblin problem" : "I'm negotiating with the goblins");
	    if (option == -1)
		return;
	    player.setBusy(true);
	    sleep();
		...
		if (stage == COMPLETE) {
		    ...
		    sendChat("Hard work like that deserves a beer", npc, player, 2000);
		    player.getActionSender().sendMessage("The bartender hands you a free beer");
		    giveItem(player, 193);
		    sleep(1200);
		    ...
		    sendChat("If you sort it out, come back and I'll shout you a beer on the house", npc, player, 1200);
		    player.setBusy(false);
		    npc.unblock();
		    return;
		}
	...
	int stage = getQuestStage(player);
	final Npc wartface = World.getWorld().getNpc(WARTFACE_ID, 314, 330, 441, 457);
	...
	int stage = player.getQuestStage(this);
	final Npc bentnoze = World.getWorld().getNpc(BENTNOZE_ID, 314, 330, 441, 457);

	if (stage == COMPLETE) {
	    sendChat("Now you've solved our argument we gotta think of something else to do", npc, player);
	...
	if (stage == -1 || stage == 1) {
	    ...
	    sendChat("Oh it you", npc, player, 2000);
	    sendChat("Have you got us some dark blue goblin armour?", npc, player);
		...
		sendChat("I thought that was the amour you were changing from", player, npc);
		sendChat("But never mind, anything is worth a try to avoid a war", player, npc, 1200);
	    ...
	    sendChat("Oh it you", npc, player, 2000);
	    sendChat("Have you got some light blue goblin armour yet?", npc, player);
		...
		sendChat("Yes i could see myself wearing somethin' like that", bentnoze, player);
		sendChat("It a deal then", npc, player);
		sendChat("Light blue it is", npc, player);
		sendChat("Thank you for sorting our argument", npc, player, 1200);
		player.setQuestStage(this, COMPLETE);
		player.incQuestPoints(QUEST_POINTS);
		player.incExp(12, REWARD_EXP, false);
		sleep();
		sendMessage(player, "General Wartface gives you a gold bar as thanks");
		giveItem(player, 172);
		player.setBusy(false);
		npc.unblock();
		bentnoze.unblock();
	...
	player.setBusy(false);
	int option = getMenuOption(player, "Why are you arguing about the colour of your armour?", "Wouldn't you prefer peace?", "Do you want me to pick an armour colour for you?");
	if (option == -1)
	    return;
	player.setBusy(true);
	sleep();
	...
	if (bentnoze == null) {
	    player.getActionSender().sendMessage("Quest error: Bentnoze missing");
	    player.setBusy(false);
	    return;
	}

	if (option == 0) {
	    queueChat(npc, player, "We decide to celebrate goblin new century", "By changing the colour of our armour", "Light blue get boring after a bit", "And we want change", "Problem is they want different change to us");
	    player.setBusy(false);
	    npc.unblock();
	    return;
	} else if (option == 1) {
	    sendChat("Yeah peace is good as long as it is peace wearing green armour", npc, player);
	...
	final Npc wartface = World.getWorld().getNpc(WARTFACE_ID, 321, 445, 326, 449);
	final Npc bentnoze = World.getWorld().getNpc(BENTNOZE_ID, 321, 445, 326, 449);
