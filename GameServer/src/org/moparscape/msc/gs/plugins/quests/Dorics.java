...
public class Dorics extends Quest {
	...
	} else if (action == action.ITEM_USED_ON_OBJECT) {
	    if (args.length < 2 || !(args[0] instanceof InvItem) || !(args[1] instanceof GameObject))
		return;

	    final InvItem item = (InvItem) args[0];
	    final GameObject obj = (GameObject) args[1];
	}
    }
	...
	if (stage == -1) {
	    ...
	    sendChat("I'm sick of people sneaking into my house to use my anvils!", npc, player);
	    sendChat("But they're the only anvils in this part of town", player, npc);
	    sendChat("It's very selfish of you to keep them all to yourself", player, npc);
	    sendChat("They're my anvils and it's my house", npc, player);
	    sendChat("So unless they earn my respect as a smither, I'll be as selfish as I want!", npc, player);

	    player.setBusy(false);
	    int option = getMenuOption(player, "How can I earn your respect?", "No wonder no one likes you", "But... I'm bigger than you");
	    if (option == -1)
		return;
	    player.setBusy(true);
	    sleep();
		...
		player.setBusy(false);
		option = getMenuOption(player, "I don't have time", "I don't work for little people", "Alright, what do you need?");
		if (option == -1)
		    return;
		player.setBusy(true);
		sleep();
		...
		if (option == 0) {
		    sendChat("Fine, leave me to it then. But don't expect me to let you use my anvils!", npc, player, 1200);
		    player.setBusy(false);
		    npc.unblock();
		    return;
		} else if (option == 1) {
		    sendChat("I'm vertically challenged!", npc, player);
		    sendChat("Get out of here before I smelt an iron bar to your ass", npc, player, 1200);
		    player.setBusy(false);
		    npc.unblock();
		    return;
		} else if (option == 2) {
		    sendChat("I need six pieces of clay, four copper ores and two iron ores", npc, player);
		    sendChat("Save me some time and bring those back here for me", npc, player);
		    sendChat("And I'll let you use my anvils any time you need to", npc, player);

		    player.setBusy(false);
		    option = getMenuOption(player, "I'll get to it then", "Maybe later");
		    if (option == -1)
			return;
		    player.setBusy(true);
		    sleep();
			...
			sendChat("I'll just wait here for you to come back then", npc, player, 1200);
			player.setBusy(false);
			npc.unblock();
			return;
		    } else {
			sendChat("Fine, but don't expect me to let you use my anvils in the meantime!", npc, player, 1200);
			player.setBusy(false);
			npc.unblock();
			return;
		    }
		...
		sendChat("I'm big where it counts, " + (player.isMale() ? "buddy" : "missy") + "!", npc, player, 1200);
		player.setBusy(false);
		npc.unblock();
		return;
	    }
	    ...
	    player.setBusy(false);
	    int option = getMenuOption(player, "Yes, I have", "No, not yet", "I forgot what to get!");
	    if (option == -1)
		return;
	    player.setBusy(true);
	    sleep();
		...
		if (countItem(player, 149) >= 6 && countItem(player, 150) >= 4 && countItem(player, 151) >= 2 && stage != COMPLETE) {
		    sendChat("Excellent! Hand 'em over, then", npc, player);
		    player.getActionSender().sendMessage("You hand Doric the materials");
		    for (int i = 0; i < 6; i++)
		    ...
		    sleep();
		    sendChat("Thanks for your help " + player.getUsername(), npc, player);
		    sendChat("Anyone who lends a hand is welcome in my workshop", npc, player);
		    sendChat("Come back and use my anvils whenever you like!", npc, player, 1200);
		    ...
		    sendChat("Liar! You're not doing a very good job of earning my trust!", npc, player, 1200);
		    player.setBusy(false);
		    npc.unblock();
		    return;
		}
		...
		sendChat("And I'll consider letting you use my anvils", npc, player, 1200);
		player.setBusy(false);
		npc.unblock();
		return;
	    } else if (option == 2) {
		sendChat("I need six pieces of clay, four copper ores and two iron ores", npc, player);
		sendChat("Oh right, thanks. I'll get back to it then", player, npc, 1200);
		player.setBusy(false);
		npc.unblock();
		return;
	    }
