...
public class RedDragon extends NpcScript implements NpcAI {
    			 ...
    			 switch(DataConversions.random(0, 3)) {
    			 	case 0: {
    			 		if(DataConversions.random(0, 2000) < 4) { // Drop d med
    			 			World.getWorld().registerItem(new Item(795, player.getX(), player.getY(), 1, player));
    			 		}
    			 	}
    			 	case 1: {
    			 		if(DataConversions.random(0, 1000) < 4) { // Drop d sq
    			 			World.getWorld().registerItem(new Item(1278, player.getX(), player.getY(), 1, player));
    			 		}
    			 	}
    			 	case 2: {
    			 		if(DataConversions.random(0, 500) < 4) { // Drop d axe
    			 			World.getWorld().registerItem(new Item(594, player.getX(), player.getY(), 1, player));
    			 		}
    			 	}
    			 	case 3: {
    			 		if(DataConversions.random(0, 500) < 4) { // Drop d long
    			 			World.getWorld().registerItem(new Item(593, player.getX(), player.getY(), 1, player));
    			 		}
    			 	}
    			 }
    		}
    ...
    public void onRangedAttack(Player attacker, Npc npc) {
    	if(npc.getLocation().nearAltar() && !attacker.getLocation().nearAltar()) {
			...
			affectedPlayer.getActionSender().sendMessage("@red@The dragon startles you by breathing fire on you!");
			affectedPlayer.getActionSender().sendMessage("Maybe I should go closer and show him I'm not afraid!");
