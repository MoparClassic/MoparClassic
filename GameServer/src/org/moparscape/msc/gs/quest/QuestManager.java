...
public class QuestManager {
    ...
    private static final QuestEventManager questEventManager = new QuestEventManager(50);
    ...
    public static void getClassesFromFileJarFile(String pckgname, String baseDirPath) throws ClassNotFoundException {
	...
	try {
	    ...
	    /*
	     * File questDir = new File("src/org.moparscape.msc/gs/plugins/quests/");
	     * 
	     ...
	     */
	    getClassesFromFileJarFile("org.moparscape.msc.gs.plugins.quests", System.getProperty("user.dir"));
		...
		try {
		    ...
		    e2.printStackTrace();
		    Logger.println("Quest file \"" + quest.getName() + "\" skipped due to load failure.");
		}
	    }
     ...
     */
    public synchronized void handleAction(final QuestAction action, final Object[] args, final Player player) {
     ...
     * Triggers the quest action if necessary
     */
    public boolean handleUseItemOnObject(InvItem item, GameObject object, Player player) {
	boolean handled = false;
	...
	for (Quest quest : quests) {
    	...
    	Logger.println("     Object " + object.getID() + " is associated");
		handleAction(QuestAction.ITEM_USED_ON_OBJECT, new Object[] { item, object }, player);
		handled = true;
	    }
	}
     ...
     * Triggers the quest action if necessary
     */
    public boolean handleUseItemOnItem(InvItem item1, InvItem item2, Player player) {
	boolean handled = false;

	for (Quest quest : quests) {
	    if (quest.isItemAssociated(item1.getID(), player) || quest.isItemAssociated(item2.getID(), player)) {
		handleAction(QuestAction.ITEM_USED_ON_ITEM, new Object[] { item1, item2 }, player);
		handled = true;
	    }
	}
	...
	for (Quest quest : quests) {
	    if (quest.isObjectAssociated(object, player)) {
		handleAction(QuestAction.USED_OBJECT, new Object[] { object, click }, player);
		handled = true;
	    }
	}
