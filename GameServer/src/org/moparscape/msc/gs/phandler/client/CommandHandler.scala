...
import org.moparscape.msc.gs.core.{ ClientUpdater, GameEngine }
import org.moparscape.msc.gs.db.DataManager
import org.moparscape.msc.gs.model.{ InvItem, Player, Point, World }
...
class CommandHandler extends PacketHandler {
  ...
  def handleCommand(cmd: String, args: Array[String], p: Player) {
    ...
    cmd match {
      ...
      case "reloadshops"       => reloadShops(p, world)
  ...
  def reloadShops(p: Player, world: World) {
    world.getShops.clear
    import scala.collection.JavaConversions._
    var field = Instance.getClass.getDeclaredField("MODULE$")
    field.setAccessible(true)
    var method = Instance.getClass.getDeclaredMethod("createDataStore")
    method.setAccessible(true)
    var ds = method.invoke(field.get(null)).asInstanceOf[DataStore]
    ds.loadShops.toList.foreach { shop => world.registerShop(shop.toShop) }
    ds.dispose
    message(p, "Reloaded shops.")
  }
